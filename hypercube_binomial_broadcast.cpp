...
int lowest_non_zero_bit(int order, int code){ // we take the convention from the course, that the lowest nonzero bit of 0 is 1 << i where i == "order of the gray code", see around page 60 
  if(code == 0)
    return order;
  else{
    int temp = code;
...
int main(int argc, char * argv[]){
  ...
  vector<int> fromGray = reverse_lookup(&toGray); // we build a reverse lookup table from the Gray codes of all nodes so as to be able to retrieve their actual rank in constant time
  ...
  if (rank==0){
    ...
    for(int i=0;i< rootNeighbors.size();i++){  
      ...
      if (neighbRank<size){ // remember we "rounded up" to the smallest hypercube containing all nodes, so we need to check this is an actual neighbor
    ...
    int grayPredecessor = (grayRank - (1 << lnb)); // works even if the predecessor is zero :-)
